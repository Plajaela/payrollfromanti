...
export function SlipModal({ isOpen, onClose, dateRangeStr, data }: SlipModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="พรีวิวสลิปเงินเดือน (ส่ง LINE)">
      <div className="flex flex-col items-center max-h-[80vh] overflow-y-auto w-full max-w-md mx-auto relative pb-20 sm:pb-4">
        
        {/* The Slip Card (What will be captured) */}
        <div 
          ref={slipRef} 
          className="bg-white w-[350px] p-6 shadow-sm border border-gray-100 font-sans relative overflow-hidden shrink-0"
        >
          {/* Header */}
      ...
      <div className="p-4 bg-white/80 backdrop-blur-sm border-t border-gray-100 flex gap-2">
        <Button onClick={onClose} variant="secondary" className="flex-1 py-3.5">
          ปิด
        </Button>
        <Button onClick={handleDownload} disabled={isGenerating} className="flex-1 py-3.5 bg-emerald-600 hover:bg-emerald-700 shadow-emerald-200 text-white flex items-center justify-center gap-2">
          {isGenerating ? <Loader2 className="w-5 h-5 animate-spin" /> : <Download className="w-5 h-5" />}
          {isGenerating ? 'กำลังสร้าง...' : 'บันทึกรูปสลิป'}
        </Button>
      </div>
    </Modal>
  );
